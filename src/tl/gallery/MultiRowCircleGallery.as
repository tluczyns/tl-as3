	...
	public class MultiRowCircleGallery extends EventDispatcher implements IRenderable {
		...
		public function setArrCountRenderableInRow(arrCountRenderableInRow: Array): void {
			...
			for (var i: uint = 0; i < this.vecRenderable.length; i++) {
				...
				else if (renderableInRow is IMultiItemGallery)
						this.containerItem.addChild(DisplayObject(IMultiItemGallery(renderableInRow)));
			}
		}
		...
		public function set numSelected(value: int): void {
			...
			//if ((this._numSelected != -1) && (this.vecRenderable[this._numSelected] is IMultiItemGallery)) IMultiItemGallery(this.vecRenderable[this._numSelected]).selected = false;
			this._numSelected = value;
			//if (this.vecRenderable[this._numSelected] is IMultiItemGallery) IMultiItemGallery(this.vecRenderable[this._numSelected]).selected = true;	
		...
		public function destroy(): void {
			...
			for (var i: int = 0; i < this.vecRenderable.length; i++) {
				...
				if (this.isRenderableInRow(i, numRenderableInRow))
					if ((renderable is IMultiItemGallery) && (this.containerItem.contains(DisplayObject(IMultiItemGallery(renderable))))) this.containerItem.removeChild(DisplayObject(IMultiItemGallery(renderable)));
				renderable = null;
			}
